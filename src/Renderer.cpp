...
void Renderer::RunImGuiSettings()
{
	...
	if (m_LaunchParams.rendererType == RendererType::RestirPt || m_LaunchParams.rendererType == RendererType::ConditionalRestir)
	{
		...
		if (m_LaunchParams.rendererType == RendererType::ConditionalRestir)
		{
			...
			m_LaunchParams.restir.gatherM = std::clamp<int>(m_LaunchParams.restir.gatherM, 1, 32);
...
void Renderer::RunImGuiPrefixStats()
{
	...
	ImGui::Checkbox("Show Prefix Entry Contrib", &m_LaunchParams.restir.showPrefixEntryContrib);
