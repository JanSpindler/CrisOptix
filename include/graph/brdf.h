...
__device__ constexpr float CDot(const glm::vec3& a, const glm::vec3& b)
{
    return glm::clamp(glm::dot(a, b), 0.0f, 1.0f);
}

__device__ constexpr float FresnelSchlick(const float F0, const float VdotH)
{
    return F0 + (1.0f - F0) * glm::pow(glm::max(0.0f, 1.0f - VdotH), 5.0f);
}

__device__ constexpr glm::vec3 FresnelSchlick(const glm::vec3& F0, const float VdotH)
{
    return F0 + (glm::vec3(1.0f) - F0) * glm::pow(glm::max(0.0f, 1.0f - VdotH), 5.0f);
}

__device__ constexpr float D(const glm::vec3& h, const float ax, const float ay) 
{
    if (h.z < 0) { return 0.0f; }
    return 1 / (PI * ax * ay) * 1 / P2((P2(h.x / ax) + P2(h.y / ay) + P2(h.z)));
}

__device__ constexpr float Lambda(const glm::vec3& v, float ax, float ay) 
{
    return (-1 + sqrt(1 + (P2(v.x * ax) + P2(v.y * ay)) / P2(v.z))) / 2;
}

__device__ constexpr float G2(const glm::vec3& to_v, const glm::vec3& to_l, const glm::vec3& h, float ax, float ay) 
{
    if (dot(to_v, h) <= 0 || dot(to_l, h) <= 0) { return 0.0f; }
    return 1 / (1 + Lambda(to_v, ax, ay) + Lambda(to_l, ax, ay));
}

__device__ constexpr glm::mat3 World2Tan(const glm::vec3& n, const glm::vec3& tan, const glm::vec3& bitan)
{
...
extern "C" __device__ BrdfResult __direct_callable__ggx(const SurfaceInteraction& interaction, const glm::vec3& outDir)
{
    ...
    // <n, l>
    const float clampedNdotL = glm::max(
        0.0f, 
