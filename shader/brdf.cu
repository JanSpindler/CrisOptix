...
static constexpr __forceinline__ __device__ float P2(const float x)
{
    return x * x;
}

static constexpr __forceinline__ __device__ float P4(const float x)
{
    return P2(P2(x));
}

static constexpr __forceinline__ __device__ float CDot(const glm::vec3& a, const glm::vec3& b)
{
    return glm::clamp(glm::dot(a, b), 0.0f, 1.0f);
}

static constexpr __forceinline__ __device__ float FresnelSchlick(const float F0, const float VdotH)
{
    return F0 + (1.0f - F0) * glm::pow(glm::max(0.0f, 1.0f - VdotH), 5.0f);
}

static constexpr __forceinline__ __device__ glm::vec3 FresnelSchlick(const glm::vec3& F0, const float VdotH)
{
    return F0 + (glm::vec3(1.0f) - F0) * glm::pow(glm::max(0.0f, 1.0f - VdotH), 5.0f);
}

static constexpr __forceinline__ __device__ float D(const glm::vec3& h, const float ax, const float ay)
{
    if (h.z < 0) { return 0.0f; }
    return 1 / (PI * ax * ay) * 1 / P2((P2(h.x / ax) + P2(h.y / ay) + P2(h.z)));
}

static constexpr __forceinline__ __device__ float Lambda(const glm::vec3& v, float ax, float ay)
{
    return (-1 + sqrt(1 + (P2(v.x * ax) + P2(v.y * ay)) / P2(v.z))) / 2;
}

static constexpr __forceinline__ __device__ float G2(const glm::vec3& to_v, const glm::vec3& to_l, const glm::vec3& h, float ax, float ay)
{
    if (dot(to_v, h) <= 0 || dot(to_l, h) <= 0) { return 0.0f; }
    return 1 / (1 + Lambda(to_v, ax, ay) + Lambda(to_l, ax, ay));
}

static constexpr __forceinline__ __device__ glm::mat3 World2Tan(const glm::vec3& n, const glm::vec3& tan, const glm::vec3& bitan)
{
...
static constexpr __forceinline__ __device__ glm::vec3 GetFromTexIfPossible(
    const bool hasTex, 
...
static constexpr __forceinline__ __device__ float GetFromTexIfPossible(
    const bool hasTex,
...
static constexpr __forceinline__ __device__ float D_GGX(const float NdotH, const float roughness)
{
...
static constexpr __forceinline__ __device__ float V_SmithJointGGX(float NdotL, float NdotV, float roughness)
{
    float a2 = roughness * roughness;
...
static constexpr __forceinline__ __device__ glm::mat3 ComputeLocalFrame(const glm::vec3& localZ)
{
    float x = localZ.x;
    float y = localZ.y;
    float z = localZ.z;
